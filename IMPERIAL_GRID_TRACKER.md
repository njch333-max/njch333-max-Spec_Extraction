...
- Current structural target: rerun `job 64` after fixing the downstream postprocess rollbacks found on fresh runs `2213` and `2214`. The deployed grid/row assembler correctly reassigned the weak-boundary leading `GPO` fragment into `ACCESSORIES`, but parser postprocess then trimmed the accepted `GPO` prefix back out of the final snapshot. A later rerun also showed handle label contamination (`Momo HANDLES oval`) causing LAUNDRY handle summary pollution. Local fixes now preserve accepted `GPO -> ACCESSORIES` evidence and repair handle label/value spillover before display/summary.
- Target order:
