...
- `2026-04-17`: Fresh `job 64 / run 2220` showed page 4/5 `HANDLES` and `FEATURE CABINETRY` had clean `By Imperial` supplier cells in provenance, but final room/checklist values dropped or demoted the supplier. Local fix now treats `By Imperial` as a recognized Imperial supplier and backfills empty supplier fields from `layout_supplier_text` / supplier-cell provenance before postprocess finalization. Verification: targeted supplier-cell tests passed, `python -m compileall App tests tools` passed, and `839` smoke tests passed. Live rerun remains pending.
