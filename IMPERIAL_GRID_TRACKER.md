...
- Current structural target: rerun `job 64` after fixing the downstream postprocess/display rollbacks found on fresh runs `2213`, `2214`, `2215`, `2216`, `2217`, `2218`, `2219`, and `2220`. The deployed grid/row assembler correctly reassigned the weak-boundary leading `GPO` fragment into `ACCESSORIES`, but parser postprocess then trimmed the accepted `GPO` prefix back out of the final snapshot. Later reruns exposed handle label contamination (`Momo HANDLES oval`), a provenance-only case where the final label was already `HANDLES` but the valid `Momo` brand prefix still lived in the label-cell provenance, visual-subrow rebuild trimming that accepted prefix back out, display/checklist rendering using suffix-only visual fragments, a boundary-straddling size prefix where `450mm BIN` put the `450mm` value fragment into the `AREA / ITEM` label, the same `450mm` prefix being lost from display/checklist visual fragments after raw rows were correct, and `By Imperial` supplier cells being present in provenance but absent from final room/checklist values. Local fixes now preserve accepted `GPO -> ACCESSORIES` evidence, repair handle label/value spillover before display/summary, move size prefixes such as `450mm` back to the value side through raw rows, visual-fragment display, and PDF-QA checklist values, and backfill empty supplier fields from clean supplier-cell provenance such as `By Imperial`.
- Target order:
...
- `2026-04-17`: Fresh `job 64 / run 2220` showed page 4/5 `HANDLES` and `FEATURE CABINETRY` had clean `By Imperial` supplier cells in provenance, but final room/checklist values dropped or demoted the supplier. Local fix now treats `By Imperial` as a recognized Imperial supplier, backfills empty supplier fields from `layout_supplier_text` / supplier-cell provenance before postprocess finalization, and removes exact duplicate `notes == supplier` values to avoid `[By Imperial] ... (By Imperial)`. Verification: targeted supplier-cell tests passed, `python -m compileall App tests tools` passed, and `839` smoke tests passed. Live rerun remains pending.
