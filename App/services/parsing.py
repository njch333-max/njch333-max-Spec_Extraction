...
def _imperial_room_from_section(section: dict[str, Any]) -> RoomRow:
    ...
    if layout_overlay.get("toe_kick"):
        row.toe_kick = _merge_lists(row.toe_kick, _coerce_string_list(layout_overlay["toe_kick"]))
    ...
    for document in documents:
        ...
        for page in document.get("pages", []):
            ...
            if laundry_room_match:
                ...
                tap_match = re.search(
                    r"(?is)((?:PHOENIX\s+)?ARLO\s+SINK\s+MIXER\s+GOOSENECK\s+200MM[_\s-]*CHROME\s+\(151-7310-00\)|(?:CAROMA\s+)?HUSK\s+RETRACTABLE\s+\(91102C4A\))",
                    laundry_body,
                )
                if sink_match:
...
def _looks_like_clarendon_false_appliance(row: dict[str, Any]) -> bool:
    ...
    if make.lower() == "polytec":
        return True
    if model.upper() in {"DRW630", "SPACE20MM", "SPACE", "M/W SPACE", "MW SPACE"}:
        return True
        ...
        filtered.append(
            {
                ...
                "make": "",
                "model_no": placeholders["dishwasher"],
                "product_url": "",
                "spec_url": "",
                "manual_url": "",
                "website_url": "",
                ...
                "source_file": "",
                "page_refs": "",
                "evidence_snippet": f"Dishwasher Make: {placeholders['dishwasher']}",
                "confidence": 0.7,
            }
        )
        ...
        filtered.append(
            {
                ...
                "make": "",
                "model_no": placeholders["microwave"],
                "product_url": "",
                "spec_url": "",
                "manual_url": "",
                "website_url": "",
                ...
                "source_file": "",
                "page_refs": "",
                "evidence_snippet": f"Microwave Make: {placeholders['microwave']}",
                "confidence": 0.7,
            }
        )
...
def _imperial_apply_compact_section_room_enrichment(row: dict[str, Any], section: dict[str, Any] | None) -> None:
    ...
    if room_key in {"kitchen", "kitchen_laundry"}:
        ...
        if kitchen_handles:
            ...
            row["handles"] = _imperial_finalize_handle_entries(_coerce_string_list(row.get("handles", [])))
        ...
        material = _imperial_extract_compact_material_block(
            section_text,
            r"BASE\s+CABINETRY\s+COLOUR\b",
            (r"FEATURE\s+FLOATING\s+SHELVES\b", r"KICKBOARDS?\b", r"HANDLES?\b", r"DESIGNER:", r"ALL COLOURS SHOWN"),
        )
        if material:
            row["door_colours_base"] = material
