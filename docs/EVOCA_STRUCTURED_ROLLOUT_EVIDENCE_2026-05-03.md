...
| 77 | 38148 | EVOC447 | 2330 | local-01bbe966 | 6 | 7 | none | none | none | 0 | PASS with caveat |
...
- EVOC447 / job 77 / run 2330: original rollout evidence clean for hard
  exclusions and key PDF-truth fields.

## Caveats

1. EVOC447 latest production run `2330` predates the Section 15
   `Drawers/Hinges = Soft Close` derivation, so its current live snapshot still
   shows blank drawer/hinge fields. The current private-PDF e2e gate covers
   EVOC447 on the current code line, but if the rollout bar requires every live
   sample to be rerun under the newest build, rerun job 77 before flipping the
   default flag.
2. Several production samples were generated by earlier local builds. The
   current code line is protected by the private-PDF e2e gate, but live evidence
   is not uniformly same-build evidence.
3. The self-hosted GitHub private-PDF runner setup is documented but not yet
...
1. Optional but stricter: rerun EVOC447 under the current build to close the
   live-build caveat.
2. If EVOC447 is rerun cleanly, flip the default flag to ON in a small isolated
   change.
3. Deploy, watch the first new Evoca job, and keep a 24-hour rollback window.
4. Do not remove the flag until at least one week of clean Evoca production
