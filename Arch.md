...
13d. Imperial Raw Spec List Excel prefers `rooms[].v6_review_rows` when present, preserving the Claude/v6 source item boundaries, row wording, and notes before parser finalization. It falls back to `rooms[].material_rows` for older snapshots. Its `By Section` rows use the same visual ordering contract as the Imperial raw Spec List room cards: `page_no`, then `provenance.visual_sort_key`, then `row_order` fallback. Non-Imperial snapshots flatten room fields, appliances, special sections, others, and warnings into `By Section`.
...
13i-2. Imperial raw Spec List room-card flattening has a HANDLES-only source-row supplement from `rooms[].v6_review_rows`. This is a presentation recovery for snapshots where final `material_rows` collapsed multiple source handle rows into one row while `v6_review_rows` still preserves the separate source item boundaries. The supplement runs before room-card rendering and before `Material Summary -> Handles`, dedupes by `(area, page, row_order)`, visual sort key, and row content, and does not mutate stored snapshots or apply to non-HANDLES rows. Supplemented rows are normalized into the existing HANDLE `display_groups` presentation shape so the supplier cell renders as the bold continuous header, specs render as source line breaks, and notes remain row-level muted parenthetical text. When the source supplier cell contains status lines such as `SUPPLIED BY CLIENT` and `INSTALLED BY IMPERIAL`, those lines remain one supplier header rather than becoming separate supplier groups.
