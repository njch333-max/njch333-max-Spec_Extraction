...
7. Run a builder-finalizer dispatch stage:
   ...
   - the Spec List display treats Evoca structured snapshots as a formatted presentation surface: raw `special_sections` evidence cards are hidden, incomplete Section 20 wet-area accessory/shower/bath/toilet rows are suppressed, source-labeled room values are compacted into legacy-style material rows, and included finish-section values can feed visible flooring/splashback rows without mutating the stored snapshot or row provenance
