...
7. Run a builder-finalizer dispatch stage:
   ...
   - the standalone `evoca_structured_extractor.py` spike is not wired into this production path yet; its current EVOC447/467/471/473/482/436/449/479/480 work is limited to source-native JSON/QA Excel output, room-note boundary handling, group-bounded text rescue, group-bounded raw-text fallback, safe table-overflow handling that avoids fake `Continuation` business rows, narrow raw-text anchor synthesis for table-dropped Evoca group anchors with separate same-page and cross-page diagnostics, extended terminal group anchors such as `Not Applicable - by owner after handover`, raw-text cursor alignment across repeated terminal/skipped groups, preservation of label-like product wording inside value columns, Drawers `Standard` / `Pot` / `Bin` wrap pairing, no-dash empty-value group headings such as EVOC436 `Bathroom / Shower`, no-child-label multiline group anchors such as EVOC479 `Alarm System`, and an output filter that skips sections 16/18/19/21/22 while still treating their headings as hard section boundaries.
   - the Evoca structured adapter is specified in `docs/EVOCA_ADAPTER_WIRING_SPEC.md` and implemented offline in `App/services/evoca_structured_adapter.py`; it maps `evoca_structured_v0` JSON into `SnapshotPayload` through a separate adapter boundary, preserves source row provenance, hard-excludes sections 16/18/19/21/22, routes terminal values verbatim without retaining rooms by them, and avoids parent-agnostic field mapping such as a single global `Type` handler, but it is not yet connected to production dispatch
