...
7. Run a builder-finalizer dispatch stage:
   ...
   - `tests/test_evoca_end_to_end.py` provides a privacy-safe real-PDF e2e harness for Evoca: the code is committed, while customer PDFs are supplied privately through `EVOCA_E2E_PDF_DIR` or the ignored `tests/fixtures/evoca_real_pdfs` directory; `tools/run_evoca_real_pdf_e2e.ps1` is the gating runner and requires all nine validated PDFs unless `-AllowSkip` is explicitly used; `.github/workflows/ci.yml` can run the gate on a self-hosted Windows runner with an external private fixture folder, and `tools/setup_github_evoca_runner.ps1` helps configure that runner after GitHub provides a registration token
