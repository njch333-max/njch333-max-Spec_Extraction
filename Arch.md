...
7. Run a builder-finalizer dispatch stage:
   ...
   - the standalone `evoca_structured_extractor.py` spike is not wired into this production path yet; its current EVOC447/467/473 work is limited to source-native JSON/QA Excel output, room-note boundary handling, group-bounded text rescue, group-bounded raw-text fallback, safe table-overflow handling that avoids fake `Continuation` business rows, narrow cross-page raw-text synthesis for table-dropped Evoca group anchors, extended terminal group anchors such as `Not Applicable - by owner after handover`, raw-text cursor alignment across repeated terminal/skipped groups, and preservation of label-like product wording inside value columns.
