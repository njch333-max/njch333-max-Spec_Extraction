...
7. Run a builder-finalizer dispatch stage:
   ...
   - the Evoca structured path is wired into `build_spec_snapshot` as a fast path before the legacy layout pipeline when the website Builder record is `Evoca` and either `SPEC_EXTRACTION_ENABLE_EVOCA_STRUCTURED` is explicitly enabled or the current run requests the `Evoca Structured` profile; the flag defaults off for controlled rollout; it runs `evoca_structured_extractor.py`, then maps `evoca_structured_v0` JSON through `App/services/evoca_structured_adapter.py`
   - Evoca Job Workspace spec runs expose two per-run controls, `Evoca Structured` and `Heuristic Only`; these persist as distinct `run_kind` values, route through the worker to `build_spec_snapshot(..., evoca_structured_mode=...)`, and override the global Evoca flag only for that run without changing environment settings
   - the Evoca structured adapter is specified in `docs/EVOCA_ADAPTER_WIRING_SPEC.md`; it maps `evoca_structured_v0` JSON into `SnapshotPayload` through a separate adapter boundary after parser-side row-local repairs such as splitting same-line child label/value text (`Model Burazzo ...`, `Type Undermount`) back into `label` / `value` pairs and keeping terminal group-anchor child rows blank even when shifted values follow, preserves source row provenance, hard-excludes sections 16/18/19/21/22 including both observed `18 AIR-CONDITIONING` / `19 PLUMBING & GAS` and `18 PLUMBING & GAS` / `19 AIR-CONDITIONING` heading variants, strips loaded page text from `source_documents`, routes terminal values verbatim without retaining rooms by them, includes Benchtops `Colour & Finish` / `Finish` in canonical bench-top fields, derives `Drawers` / `Hinges` soft-close states from source-backed Section 15 cabinet notes, and avoids parent-agnostic field mapping such as a single global `Type` handler
   - the Spec List display treats Evoca structured snapshots as a formatted presentation surface: raw `special_sections` evidence cards are hidden, incomplete Section 20 wet-area accessory/shower/bath/toilet rows are suppressed, source-labeled room values such as `Manufacturer & Model` are compacted into legacy-style material rows, source-backed handle decisions such as `Client to supply & install after handover` or `Finger Pulls to Kitchen` remain visible while exact `#N/A`/`Not Applicable` markers are filtered, and included finish-section values can feed visible flooring/splashback rows without mutating the stored snapshot or row provenance
   - the Raw Snapshot Excel export has an Evoca structured branch: snapshots with `parser_strategy = evoca_structured_v0` export as a source-native workbook (`Summary` plus one sheet per included source section) using stored `material_rows` / appliance provenance and the Claude-like `Page / Order / Room / Group / Label / Value / Anchor / Source Text` layout; it does not re-run the PDF parser and does not expose a generic `Special Sections` sheet
   - the Evoca structured fast path skips legacy layout, Docling, Heavy Vision, OpenAI merge, and builder-specific polish; unsupported multi-source input, extractor exceptions, or no source-backed evidence emit `evoca_structured_fallback` and return to the legacy Evoca pipeline
   - `tests/test_evoca_end_to_end.py` provides a privacy-safe real-PDF e2e harness for Evoca: the code is committed, while customer PDFs are supplied privately through `EVOCA_E2E_PDF_DIR` or the ignored `tests/fixtures/evoca_real_pdfs` directory; `tools/run_evoca_real_pdf_e2e.ps1` is the gating runner and requires all required private Evoca regression PDFs unless `-AllowSkip` is explicitly used; `.github/workflows/ci.yml` can run the gate on a self-hosted Windows runner with an external private fixture folder, and `tools/setup_github_evoca_runner.ps1` helps configure that runner after GitHub provides a registration token
...
13f. Imperial v6 adapter-owned display payload is now split by purpose: source-backed row-payload `display_lines` remains the flat downstream fallback surface, while optional HANDLES-only `display_groups` carries grouped `supplier + lines[]` data for grouped UI rendering. Helper-produced room-card rendered lines are a separate display layer and must not be treated as the same summary input surface.
...
13h-1. The flat Imperial handle-summary path is now explicitly source-backed. `parsing.py` `_imperial_handle_subitems_for_row(...)` may consume row-payload `display_lines`, raw row text, and provenance fallback sources, but it must not consume helper-produced `rendered_line`. `main.py` `_imperial_material_row_handle_summary_candidates(...)` then prefers, in order, grouped `display_groups`, `handle_subitems`, source-backed v6 `display_lines`, synth/review `layout_value_text`, raw `supplier + specs_or_description + notes`, and `page_text_handle_block` as the last fallback.
...
7a. Because the v6 fast path bypasses the legacy recovery branches, source-row data loss on non-room pages must be fixed at the v6 extractor/adapter boundary rather than hidden by downstream display cleanup. Job 38 run 2369 exposed this for page 5 `DISHWASHER` and page 8 `TAPWARE (KITCHEN)`; the layer (a) extractor fix is implemented in `App/services/pdf_to_structured_json.py` and production-verified on job 38 run 2370 (`Imperial v6`, build `local-6b508c03`). The fix consists of (i) a row-label guard in `extract_page_title()` that ignores `^(SINKWARE|TAPWARE)\s*\(...\)` lines so they cannot be mistaken for a new section title (so page 8 stays attached to `SINKWARE & TAPWARE`), and (ii) a scoped template-recovery pass on titled `APPLIANCES` pages that re-runs `extract_continuation_with_template()` against the same page and appends only AREA labels in `_APPLIANCE_AREA_WHITELIST` (so page 5 `DISHWASHER` is recovered when its row separator is not full-page-width). Recovered rows carry `_source.method = "titled_template_recovery"`. Task spec: `docs/BUG_IMPERIAL_V6_DISHWASHER_TAPWARE_DATA_LOSS_TASK.md`. Tests: `tests/test_v6_titled_template_recovery.py`.
7b. Job 38 page 2 added a second layer (a) row-assembly guard: matching AREA/SPECS line counts are not enough to prove a missing row separator. The v6 extractor must keep wrapped single labels such as `BASE CABINETRY COLOUR / INCLUDES SINGLE CABINET ON BAR / BACK AREA` as one row, preserving AREA-to-SPECS gap overflow words and moving source-backed qualifier text into row-local `notes`. The downstream finalizer then removes duplicated note prefixes from door-colour descriptions so the material value remains `Classic White Matt` while the note stays observable. Fixed 2026-05-18 and production-verified on job 38 run 2371 (`Imperial v6`, build `local-89aa455a`). Task spec: `docs/BUG_IMPERIAL_V6_BASE_CABINETRY_WRAPPED_LABEL_TASK.md`. Tests: `tests/test_pdf_extractor_split.py` and `tests/test_imperial_v6_adapter.py`.
...
Job 38 run 2369 added a layer-proof requirement for non-room pages: page 5 `DISHWASHER` and page 8 `TAPWARE (KITCHEN)` had to be checked in raw v6 JSON first. The 2026-05-18 investigation confirmed both rows were lost at layer (a) (the title-detection heuristic mis-classified page 8 as its own section, and the appliance row separator on page 5 was not full-page-width). Both are recovered by the `pdf_to_structured_json.py` fixes referenced in rule 7a and were production-verified on job 38 run 2370 plus latest `spec-list.xlsx`; no `imperial_v6_adapter.py` or downstream-post-processing change was needed. The Imperial primary `Tap` display exclusion is unchanged.

The same job also proved that layer (a) missing-row-separator hints need a wrapped-label exception: the page 2 base-cabinetry row is one logical source row, not three source rows. The extractor now repairs that row before review-hint splitting, and the layer (b) finalizer removes note-prefix duplication from the finalized door-colour description. Production run 2371 verified one `BASE CABINETRY COLOUR` source row and no fake `INCLUDES SINGLE CABINET ON` / `BACK AREA` label rows in the latest `spec-list.xlsx`.
