...
7. Run a builder-finalizer dispatch stage:
   ...
   - the Spec List display treats Evoca structured snapshots as a formatted presentation surface: raw `special_sections` evidence cards are hidden, while source-labeled room values are compacted into legacy-style material rows without mutating the stored snapshot or row provenance
