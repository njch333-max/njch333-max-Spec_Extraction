...
7. Run a builder-finalizer dispatch stage:
   ...
   - the future Evoca structured adapter is specified in `docs/EVOCA_ADAPTER_WIRING_SPEC.md`; it must map `evoca_structured_v0` JSON into `SnapshotPayload` through a separate adapter boundary, preserve source row provenance, keep skipped sections skipped, and avoid parent-agnostic field mapping such as a single global `Type` handler
