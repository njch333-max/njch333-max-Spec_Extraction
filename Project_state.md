...
- Evoca real-PDF e2e coverage now lives in `tests/test_evoca_end_to_end.py` and is privacy-safe by default: it skips when no private PDFs are available and uses `EVOCA_E2E_PDF_DIR` or the ignored `tests/fixtures/evoca_real_pdfs` directory when local or CI private fixtures are supplied. `tools/run_evoca_real_pdf_e2e.ps1` is the gating runner and requires all nine validated PDFs unless `-AllowSkip` is explicitly used.
