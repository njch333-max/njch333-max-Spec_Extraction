...
- Application code is implemented:
  ...
  - Imperial handle postprocess now repairs label/value spillover such as `Momo HANDLES oval`, restoring the table label to `HANDLES`, carrying the valid `Momo` prefix into the handle value, and preventing visible-separated `oval wardrobe tube` text from entering handle summary
...
- Run a targeted fresh live rerun for Imperial `job 64` after the accessory and handle postprocess rollback fixes; `run 2213` proved the overlay/parser function was correct but final snapshot postprocess still needed to preserve accepted `GPO -> ACCESSORIES`, and `run 2214` exposed handle label spillover around `Momo HANDLES oval`
