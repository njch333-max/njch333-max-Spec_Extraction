...
- Evoca adapter wiring now has a draft design spec at `docs/EVOCA_ADAPTER_WIRING_SPEC.md` and an offline pure adapter in `App/services/evoca_structured_adapter.py`. The module maps `evoca_structured_v0` JSON to `SnapshotPayload`, preserves row-local provenance, enforces the section filter, and keeps overloaded labels such as `Type` parent-group-aware; production dispatch is still not connected.
