...
- Evoca structured parser work now supports production snapshot generation through the `evoca_structured_v0` fast path and adapter boundary. The current EVOC447/467/471/473/482/436/449/479/480 pass targets source-native JSON/QA Excel plus production `SnapshotPayload` generation, with room-note boundary fixes, group-bounded text rescue, group-bounded raw-text fallback to recover values missed by both table and text-grid extraction, safe overflow handling that appends source-backed wraps while preserving unsafe extras as `Unassigned Source Text` diagnostics instead of fake `Continuation` rows, raw-text anchor synthesis for table-dropped `Benchtops` / `Basin Mixer` / `Underbench` / `Accessories` groups with same-page and cross-page diagnostics, extended terminal group anchors for source-native values such as `Not Applicable - by owner after handover`, terminal group anchors that block shifted child-value assignment, raw-text cursor alignment across repeated terminal/skipped groups plus value-column label-word preservation, same-line child label/value splitting for glued rows such as `Model Burazzo ...`, Drawers `Standard` / `Pot` / `Bin` wrap pairing with bounded raw-text fallback, no-dash empty-value group headings such as EVOC436 `Bathroom / Shower`, no-child-label multiline group anchors such as EVOC479 `Alarm System`, and an explicit output filter that recognizes but skips sections 16/18/19/21/22.
...
- Evoca real-PDF e2e coverage now lives in `tests/test_evoca_end_to_end.py` and is privacy-safe by default: it skips when no private PDFs are available and uses `EVOCA_E2E_PDF_DIR` or the ignored `tests/fixtures/evoca_real_pdfs` directory when local or CI private fixtures are supplied. `tools/run_evoca_real_pdf_e2e.ps1` is the gating runner and requires all nine validated PDFs unless `-AllowSkip` is explicitly used. `.github/workflows/ci.yml` adds standard hosted Python tests plus an optional self-hosted `evoca-fixtures` job for the private PDF gate, and `tools/setup_github_evoca_runner.ps1` validates/configures the runner once GitHub provides a registration token.
