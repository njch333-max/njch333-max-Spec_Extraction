...
- Evoca adapter wiring now has a draft design spec at `docs/EVOCA_ADAPTER_WIRING_SPEC.md`. The next approved implementation should start with an offline pure adapter from `evoca_structured_v0` JSON to `SnapshotPayload`, preserve row-local provenance, enforce the section filter, and keep overloaded labels such as `Type` parent-group-aware before any production dispatch is added.
