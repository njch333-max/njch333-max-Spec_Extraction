...
- Evoca structured parser work now supports production snapshot generation through the `evoca_structured_v0` fast path and adapter boundary. The current EVOC447/467/471/473/482/436/449/479/480 pass targets source-native JSON/QA Excel plus production `SnapshotPayload` generation, with EVOC434 added as a real-PDF hard-exclusion numbering-variant regression sample. The parser covers room-note boundaries, group-bounded text rescue, group-bounded raw-text fallback to recover values missed by both table and text-grid extraction, safe overflow handling that appends source-backed wraps while preserving unsafe extras as `Unassigned Source Text` diagnostics instead of fake `Continuation` rows, raw-text anchor synthesis for table-dropped `Benchtops` / `Basin Mixer` / `Underbench` / `Accessories` groups with same-page and cross-page diagnostics, extended terminal group anchors for source-native values such as `Not Applicable - by owner after handover`, terminal group anchors that block shifted child-value assignment, raw-text cursor alignment across repeated terminal/skipped groups plus value-column label-word preservation, same-line child label/value splitting for glued rows such as `Model Burazzo ...`, Drawers `Standard` / `Pot` / `Bin` wrap pairing with bounded raw-text fallback, no-dash empty-value group headings such as EVOC436 `Bathroom / Shower`, no-child-label multiline group anchors such as EVOC479 `Alarm System`, and an explicit output filter that recognizes but skips sections 16/18/19/21/22 including observed `18 PLUMBING & GAS` / `19 AIR-CONDITIONING` variants.
- Evoca adapter wiring now has `docs/EVOCA_ADAPTER_WIRING_SPEC.md`, the pure adapter in `App/services/evoca_structured_adapter.py`, and production dispatch wiring in `build_spec_snapshot`. When the website Builder record is `Evoca` and either `SPEC_EXTRACTION_ENABLE_EVOCA_STRUCTURED` is explicitly enabled or the current run requests the `Evoca Structured` profile, the app runs the standalone extractor and maps `evoca_structured_v0` JSON to `SnapshotPayload` before the legacy layout pipeline; the flag defaults off for controlled rollout. Evoca Job Workspace spec runs now show explicit `Evoca Structured` and `Heuristic Only` buttons, stored as per-run parser choices so operators can force either path without changing environment settings. The path preserves row-local provenance, hard-excludes sections 16/18/19/21/22, strips loaded page text from `source_documents`, routes terminal values verbatim without retaining rooms by them, keeps Benchtops `Colour & Finish` / `Finish` in canonical bench-top fields, derives `Drawers` / `Hinges` soft-close states from source-backed Section 15 cabinet notes, and keeps overloaded labels such as `Type` parent-group-aware. Spec List presentation now hides Evoca structured raw `special_sections` cards, suppresses incomplete Section 20 wet-area accessory/shower/bath/toilet rows, compacts labeled room values such as `Manufacturer & Model` into legacy-style display text, keeps source-backed handle decisions such as `Client to supply & install after handover` or `Finger Pulls to Kitchen` while filtering exact `#N/A`/`Not Applicable` markers, and uses included finish sections for visible flooring/splashback rows while preserving stored row provenance; flag-off/profile-off, unsupported source counts, extractor errors, or no source-backed evidence fall back to the legacy Evoca pipeline.
- Evoca structured Raw Snapshot Excel export now branches to a source-native workbook generated from stored snapshot provenance. It uses `Summary` plus section sheets such as `15_CABINETS`, `17_APPLIANCES...`, and `20_PLUMBING_FIXTURES...`, with `Page / Order / Room / Group / Label / Value / Anchor / Source Text` columns, room banners, and anchor/terminal row styling. It intentionally avoids the generic `By Section` and raw `Special Sections` workbook tabs for Evoca structured snapshots.
- Evoca real-PDF e2e coverage now lives in `tests/test_evoca_end_to_end.py` and is privacy-safe by default: it skips when no private PDFs are available and uses `EVOCA_E2E_PDF_DIR` or the ignored `tests/fixtures/evoca_real_pdfs` directory when local or CI private fixtures are supplied. `tools/run_evoca_real_pdf_e2e.ps1` is the gating runner and requires all required private Evoca regression PDFs unless `-AllowSkip` is explicitly used. `.github/workflows/ci.yml` adds standard hosted Python tests plus an optional self-hosted `evoca-fixtures` job for the private PDF gate, and `tools/setup_github_evoca_runner.ps1` validates/configures the runner once GitHub provides a registration token.
- The next planned builder-specific parser line is Yellowwood. The handoff is `docs/YELLOWWOOD_NEXT_CHAT_HANDOFF_2026-05-03.md`. The intended approach is Evoca-style staged delivery, not Evoca rule cloning: source-PDF inventory, `docs/YELLOWWOOD_STRUCTURED_PARSER_SPEC.md`, standalone `yellowwood_structured_v0` extractor/schema, private real-PDF e2e tests, then adapter spec/pure adapter, with production wiring or Job Workspace run buttons deferred until Jason explicitly approves that phase.
...
- Imperial raw Spec List room-card notes now preserve source-backed LF line breaks on the separate-notes path: as of 2026-04-26 commit `e0ebd37`, inline notes only fire when the visible payload is one line and notes contains no `\n`, while separate notes use `.row-note-multiline { white-space: pre-line; }`; scope is template + CSS + tests only, with no Python data-layer or `Material Summary` changes
- Imperial v6 handle backfill is now idempotent across repeated builder-finalizer passes: as of 2026-04-26 commit `9578fd7`, `App/services/parsing.py` `_imperial_reconcile_material_rows_with_room_fields(...)` skips re-emitting a synth handle row when the room already contains a `synthesized_from_room_handles` row with the same normalized `layout_value_text`; scope is parsing.py reconcile gate + tests only, leaving finalizer topology, handle identity helpers, templates, and `Material Summary` builder unchanged
- Imperial synth handle room-card display no longer double-renders notes: as of 2026-04-26 commit `23e233a`, `App/services/parsing.py` `_imperial_material_row_display_lines_for_view(...)` leaves notes out of helper-level display lines for `synthesized_from_room_handles` rows and preserves the row-level `notes` field so the room-card notes layer renders the note once; scope is parsing.py helper + tests only, leaving templates, `App/main.py`, Excel source rows, and `Material Summary` logic unchanged
- Imperial handle Material Summary is now decoupled from room-card helper text: as of 2026-04-26 commit `727b3ff`, `App/services/parsing.py` `_imperial_handle_subitems_for_row(...)` no longer consumes helper-produced `rendered_line`, and `App/main.py` `_imperial_material_row_handle_summary_candidates(...)` now prefers source-backed grouped `display_groups`, `handle_subitems`, source-backed v6 `display_lines`, synth/review `layout_value_text`, raw row text, and `page_text_handle_block` in deterministic order; scope is parsing.py + App.main + tests only, with live acceptance based on source-PDF wording rather than old HTML byte identity
- Imperial v6 job 38 run 2369 source-row data-loss task is fixed in `App/services/pdf_to_structured_json.py` (2026-05-18) and production-verified on job 38 run 2370 (`Imperial v6`, build `local-6b508c03`). `extract_page_title()` no longer treats AREA-column row labels matching `^(SINKWARE|TAPWARE)\s*\(...\)` as section titles, so page 8 `TAPWARE (KITCHEN)` rejoins `SINKWARE & TAPWARE` via the existing continuation path. Titled `APPLIANCES` pages also run a scoped template-recovery pass after grid extraction, gated by a small appliance AREA whitelist (`OVEN / COOKTOP / DISHWASHER / RANGEHOOD / MICROWAVE / FRIDGE / BAR FRIDGE / ...`), which restores the page 5 `DISHWASHER` row missed by `smart_filter_y_edges` when the row separator is not full-page-width. Live run 2370 and latest `spec-list.xlsx` contain `DISHWASHER` / `DBI364ID.S.AU` and `TAPWARE (KITCHEN)` / `TA9601CP`, with the false `TAPWARE (KITCHEN) BY CLIENT` section gone. Scope intentionally excludes kitchen base-cabinetry continuation, handle grouping, label clipping, and changing the Imperial primary `Tap` display exclusion. Task spec: `docs/BUG_IMPERIAL_V6_DISHWASHER_TAPWARE_DATA_LOSS_TASK.md`. Tests: `tests/test_v6_titled_template_recovery.py`.
- Imperial v6 job 38 page 2 base-cabinetry wrapped-label split is fixed in `App/services/pdf_to_structured_json.py` and `App/services/parsing.py` (2026-05-18) and production-verified on job 38 run 2371 (`Imperial v6`, build `local-89aa455a`). `BASE CABINETRY COLOUR / INCLUDES SINGLE CABINET ON BAR / BACK AREA` now remains one source row: AREA `BASE CABINETRY COLOUR`, specs `Classic White / Matt`, supplier `Polytec`, notes `INCLUDES SINGLE CABINET ON BAR BACK AREA`, and finalized display `Polytec - Classic White Matt`. The latest `spec-list.xlsx` has no fake `INCLUDES SINGLE CABINET ON` or `BACK AREA` label rows. This keeps true missing-row-separator splitting for rows such as job 76 pull-out blocks. Task spec: `docs/BUG_IMPERIAL_V6_BASE_CABINETRY_WRAPPED_LABEL_TASK.md`. Tests: `tests/test_pdf_extractor_split.py` and `tests/test_imperial_v6_adapter.py`.
...
| 7 | Job 38 run 2369 dropped source-backed page 5 `DISHWASHER` and page 8 `TAPWARE (KITCHEN)` from the v6 output | High | Fixed 2026-05-18 in `App/services/pdf_to_structured_json.py`; production verified on job 38 run 2370 | Layer (a) extractor fix only. Title rule + scoped APPLIANCES template recovery + appliance whitelist; tested in `tests/test_v6_titled_template_recovery.py`. Imperial `Tap` primary exclusion unchanged. |
| 8 | Job 38 page 2 split one `BASE CABINETRY COLOUR` wrapped source row into three rows | High | Fixed 2026-05-18; production verified on job 38 run 2371 | Layer (a) extractor repair plus layer (b) door-colour note-prefix dedupe. Keeps `BAR` gap word, moves `INCLUDES...BACK AREA` to notes, preserves `Classic White Matt` as value. |
...
- **Job 38 data-loss task**: fixed page 5 `DISHWASHER` plus page 8 `TAPWARE (KITCHEN)` recovery through the v6 raw extractor boundary and production-verified on run 2370. Page 2 base-cabinetry wrapped-label repair is production-verified on run 2371. Separate remaining backlog items still cover handle summary/display and label/note clipping.
...
- Start the Yellowwood dedicated parser from the handoff and a parser spec, not from production wiring. Use jobs cited in the docs (`12`, `24`, `37`) as starting references only after verifying current live source PDFs and artifacts.
