...
- Application code is implemented:
  ...
  - Imperial postprocess now corrects boundary-straddling size prefixes such as `450mm BIN`, keeping the original table label as `BIN` and moving the size token back into the value text when visible grid evidence puts it on the description side
