...
- Standalone Evoca structured parser work remains offline from production snapshot generation. The current EVOC447/467/471/473/482/436/449/479/480 pass targets source-native JSON/QA Excel only, with room-note boundary fixes, group-bounded text rescue, group-bounded raw-text fallback to recover values missed by both table and text-grid extraction, safe overflow handling that appends source-backed wraps while preserving unsafe extras as `Unassigned Source Text` diagnostics instead of fake `Continuation` rows, raw-text anchor synthesis for table-dropped `Benchtops` / `Basin Mixer` / `Underbench` / `Accessories` groups with same-page and cross-page diagnostics, extended terminal group anchors for source-native values such as `Not Applicable - by owner after handover`, raw-text cursor alignment across repeated terminal/skipped groups plus value-column label-word preservation, Drawers `Standard` / `Pot` / `Bin` wrap pairing with bounded raw-text fallback, no-dash empty-value group headings such as EVOC436 `Bathroom / Shower`, no-child-label multiline group anchors such as EVOC479 `Alarm System`, and an explicit output filter that recognizes but skips sections 16/18/19/21/22.
- Evoca adapter wiring now has a draft design spec at `docs/EVOCA_ADAPTER_WIRING_SPEC.md` and an offline pure adapter in `App/services/evoca_structured_adapter.py`. The module maps `evoca_structured_v0` JSON to `SnapshotPayload`, preserves row-local provenance, hard-excludes sections 16/18/19/21/22, routes terminal values verbatim without retaining rooms by them, and keeps overloaded labels such as `Type` parent-group-aware; production dispatch is still not connected.
