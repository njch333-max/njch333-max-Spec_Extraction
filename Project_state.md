...
- Standalone Evoca structured parser work remains offline from production snapshot generation. The current EVOC447/467/471/473/482/436/449 pass targets source-native JSON/QA Excel only, with room-note boundary fixes, group-bounded text rescue, group-bounded raw-text fallback to recover values missed by both table and text-grid extraction, safe overflow handling that appends source-backed wraps while preserving unsafe extras as `Unassigned Source Text` diagnostics instead of fake `Continuation` rows, raw-text anchor synthesis for table-dropped `Benchtops` / `Basin Mixer` / `Underbench` / `Accessories` groups with same-page and cross-page diagnostics, extended terminal group anchors for source-native values such as `Not Applicable - by owner after handover`, raw-text cursor alignment across repeated terminal/skipped groups plus value-column label-word preservation, Drawers `Standard` / `Pot` / `Bin` wrap pairing with bounded raw-text fallback, and no-dash empty-value group headings such as EVOC436 `Bathroom / Shower`.
