...
- Imperial v6 Raw Snapshot Excel `By Section` ordering now follows the raw Spec List room-card visual ordering contract: `page_no`, `provenance.visual_sort_key`, then `row_order` fallback. This keeps Excel review rows aligned with the web room-card order while leaving `Material Summary` as a category summary rather than a room-card order clone.
- Imperial v6 raw Spec List room-card flattening now supplements HANDLES-only rows from same-room `v6_review_rows` when final `material_rows` collapsed multiple source handle rows into one row. This restores source rows such as `LIP PULL HANDLES - DRAWERS` and `FEATURE LIP PULL PANTRY HANDLES` to the web room cards and `Material Summary -> Handles` without changing stored snapshots, parser output, or Excel export behavior. Supplemented rows are also mapped into the normal grouped handle display shape so supplier text is bold, specs keep source line breaks, and notes render as muted parenthetical row notes. Supplier-cell status lines like `SUPPLIED BY CLIENT` and `INSTALLED BY IMPERIAL` stay together in one supplier header.
