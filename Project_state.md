...
- Standalone Evoca structured parser work remains offline from production snapshot generation. The current EVOC447/467/473 pass targets source-native JSON/QA Excel only, with room-note boundary fixes, group-bounded text rescue, group-bounded raw-text fallback to recover values missed by both table and text-grid extraction, Bug 8 overflow handling that appends source-backed wraps while preserving unsafe extras as `Unassigned Source Text` diagnostics instead of fake `Continuation` rows, Bug 9 cross-page raw-text synthesis for table-dropped `Benchtops` / `Basin Mixer` groups, Bug 10 extended terminal group anchors for source-native values such as `Not Applicable - by owner after handover`, Bug 11 raw-text cursor alignment across repeated terminal/skipped groups plus value-column label-word preservation, and Bug 12 Drawers `Standard` / `Pot` / `Bin` wrap pairing with bounded raw-text fallback.
