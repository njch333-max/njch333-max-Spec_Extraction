...
- Imperial v6 raw Spec List room-card flattening now supplements HANDLES-only rows from same-room `v6_review_rows` when final `material_rows` collapsed multiple source handle rows into one row. This restores source rows such as `LIP PULL HANDLES - DRAWERS` and `FEATURE LIP PULL PANTRY HANDLES` to the web room cards and `Material Summary -> Handles` without changing stored snapshots, parser output, or Excel export behavior.
