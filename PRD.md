...
- `Shelf` is a dedicated room material field. Populate it only when the source explicitly ties a material or finish to shelf shelving, such as `Open Shelving ... White Melamine` or `Shelves ... Polytec ...`. Do not infer it from rail-only rows, generic fit-out notes, or unrelated material fields.
