...
- When Imperial v6 snapshots carry `rooms[].v6_review_rows`, the raw Spec List Excel export must use those rows for the Claude workbook so original v6 item boundaries, row wording, notes, and metadata survive parser finalization. `By Section` ordering must align with the raw Spec List room-card ordering by sorting Imperial rows by `page_no`, `provenance.visual_sort_key`, and then `row_order` fallback. `material_rows` remain the parser/UI row model and the fallback for older snapshots.
...
- Imperial raw Spec List rendering may supplement missing HANDLES-only source rows from `rooms[].v6_review_rows` when final `material_rows` collapsed multiple source handle rows into one display row. This recovery is scoped to room cards and `Material Summary -> Handles`, dedupes against existing `material_rows` by source/visual/content identity, does not mutate stored snapshots, and must not apply to non-HANDLES rows. Supplemented rows must use the same handle row presentation as native grouped rows: supplier text as the bold continuous header, specs/description as source-preserved wrapped lines, and notes as muted parenthetical row notes. Supplier-cell status lines such as `SUPPLIED BY CLIENT` and `INSTALLED BY IMPERIAL` remain together as one supplier header instead of being split into separate supplier groups.
