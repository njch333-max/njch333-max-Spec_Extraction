...
- Clarendon deterministic polish may restore a room-master room that earlier retention dropped when PDF text order places the schedule title after its material body, but only when the selected `Drawings and Colours` master overlay contains true material fields.
...
- Imperial v6 `HANDLES` rows may now carry two display surfaces: source-backed row-payload `display_lines` for flat summary/fallback use and structured `display_groups` for room-card rendering. `display_groups` is HANDLES-only and is allowed only when supplier-to-spec ownership is safe enough to express as grouped UI, including the accepted equal-share case where `M` handle spec lines are split evenly across `N` supplier lines. Helper-produced room-card rendered lines and flattened `display_value` are not summary input.
...
- When an Imperial handle row has no usable `handle_subitems`, `Material Summary / Handles` must fall back in this source-backed order: v6 row-payload `display_lines`, synth/review `provenance.layout_value_text`, raw `supplier + specs_or_description + notes`, then `page_text_handle_block` as the final fallback. The summary path must not depend on room-card helper formatting.
...
- The active cross-builder parser regression matrix currently includes:
  ...
  - `Simonds`: `job 19`
  - `Evoca`: `job 39`
- The Evoca structured parser now supports the production fast path through `evoca_structured_v0` JSON and the adapter boundary. Current EVOC447/467/471/473/482/436/449/479/480 work fixes source-native JSON/QA workbook behavior, including room-note boundaries, group-bounded text rescue, group-bounded raw-text fallback for values missed by both table and text-grid extraction, safe handling of table overflow text without emitting fake `Continuation` business labels, narrow raw-text anchor synthesis for table-dropped Evoca group anchors with separate same-page and cross-page diagnostics, extended terminal group anchors such as `Not Applicable - by owner after handover`, terminal group anchors that block shifted child-value assignment, raw-text cursor alignment across repeated terminal/skipped groups, preservation of label-like product wording inside value columns, splitting same-line child label/value text such as `Model Burazzo ...` back into row-local values, Drawers `Standard` / `Pot` / `Bin` wrap pairing, no-dash empty-value group headings such as EVOC436 `Bathroom / Shower`, and no-child-label multiline group anchors such as EVOC479 `Alarm System`.
- The standalone Evoca parser intentionally filters sections `16 ELECTRICAL / ALARM SYSTEM / CCTV / SOLAR PV SYSTEM`, `18 AIR-CONDITIONING`, `19 PLUMBING & GAS`, `21 MIRRORS`, and `22 WINDOW FURNISHINGS` from JSON and QA workbook output while still treating their headings as hard boundaries. Observed Evoca PDFs may swap the plumbing/gas and air-conditioning numbering to `18 PLUMBING & GAS` / `19 AIR-CONDITIONING`; those forms are also hard-excluded boundaries.
- Evoca structured adapter wiring is specified in `docs/EVOCA_ADAPTER_WIRING_SPEC.md`. `build_spec_snapshot` can attempt an Evoca structured fast path before the legacy layout pipeline when the website Builder record is `Evoca` and either `SPEC_EXTRACTION_ENABLE_EVOCA_STRUCTURED` is enabled or the current run explicitly requests the `Evoca Structured` profile; the flag defaults off for controlled rollout. The Evoca Job Workspace exposes two explicit per-run buttons, `Evoca Structured` and `Heuristic Only`, so operators can choose the structured path or force the legacy pipeline without changing environment settings. The path runs `evoca_structured_extractor.py`, maps only `evoca_structured_v0` JSON through `App/services/evoca_structured_adapter.py`, preserves row-local provenance, hard-excludes sections 16/18/19/21/22 from snapshot output, strips loaded page text from `source_documents`, routes terminal values verbatim without retaining rooms by them, routes Benchtops `Colour & Finish` / `Finish` labels into canonical bench-top fields, derives `Drawers` / `Hinges` soft-close states from source-backed Section 15 cabinet notes, and routes overloaded `Type` labels by parent group. The Spec List display hides Evoca structured raw `special_sections` cards, formats source-labeled canonical values such as `Manufacturer`, `Manufacturer & Model`, `Colour & Finish`, and `Edge Profile` into compact legacy-style room rows, suppresses incomplete Section 20 wet-area accessory/shower/bath/toilet rows, keeps source-backed handle decisions such as `Client to supply & install after handover` or `Finger Pulls to Kitchen` while filtering exact `#N/A`/`Not Applicable` markers, and derives visible flooring/splashback display from included finish sections without changing stored provenance. If the flag/profile is off, the source count is unsupported, the extractor fails, or no source-backed evidence is produced, the app falls back to the legacy Evoca pipeline.
- Evoca structured Raw Snapshot Excel export now uses a dedicated source-native workbook instead of the generic `By Section` review workbook. The workbook has a `Summary` sheet and one sheet per included Evoca source section, with Claude-like `Page / Order / Room / Group / Label / Value / Anchor / Source Text` columns, room banners, anchor styling, and no raw `Special Sections` sheet. The export reads stored snapshot provenance only; it does not re-run the PDF parser.
- Evoca real-PDF e2e coverage is defined in `tests/test_evoca_end_to_end.py`; it runs only when private PDFs are supplied through `EVOCA_E2E_PDF_DIR` or the ignored `tests/fixtures/evoca_real_pdfs` directory, so customer PDFs are not committed to source control. `tools/run_evoca_real_pdf_e2e.ps1` is the gating runner and fails when any required Evoca regression PDF is missing unless `-AllowSkip` is explicitly used. GitHub CI support uses a self-hosted Windows runner with an external private fixture folder; setup is documented in `docs/EVOCA_PRIVATE_E2E_CI_SETUP.md`, and `tools/setup_github_evoca_runner.ps1` validates/configures the runner after GitHub provides a short-lived registration token.
- The next Yellowwood parser cycle should reuse the Evoca staged delivery model rather than the Evoca business rules. Yellowwood dedicated-parser work should start with `docs/YELLOWWOOD_NEXT_CHAT_HANDOFF_2026-05-03.md`, a source-PDF inventory, and a new `docs/YELLOWWOOD_STRUCTURED_PARSER_SPEC.md`; production wiring, Job Workspace buttons, and SnapshotPayload changes are out of scope until the standalone parser, schema, and private real-PDF e2e coverage prove the source-native output.
...
- Imperial raw Spec List `Material Summary -> Handles` is now decoupled from room-card helper text. `_imperial_handle_subitems_for_row(...)` must not consume helper-produced `rendered_line`, and `_imperial_material_row_handle_summary_candidates(...)` must not use helper-only `display_value` as a primary source. The accepted flat handle-summary path is source-backed only.
...
- Imperial v6 non-room page recovery must preserve source-backed appliance and sinkware/tapware rows before presentation filtering. Job 38 run 2369 showed page 5 `DISHWASHER` and page 8 `TAPWARE (KITCHEN)` present in the source PDF but absent from the v6 output. Fixed 2026-05-18 in `App/services/pdf_to_structured_json.py` and production-verified on job 38 run 2370 (`Imperial v6`, build `local-6b508c03`): `extract_page_title()` now skips AREA-column row labels matching `^(SINKWARE|TAPWARE)\s*\(...\)` so page 8 stays inside `SINKWARE & TAPWARE` and is recovered by the existing continuation path; titled `APPLIANCES` pages additionally run a scoped template-recovery pass that appends only AREA labels in `_APPLIANCE_AREA_WHITELIST` (OVEN / COOKTOP / DISHWASHER / RANGEHOOD / MICROWAVE / FRIDGE / BAR FRIDGE / FREEZER / WINE FRIDGE / COFFEE MACHINE / STEAM OVEN / WARMING DRAWER), so page 5 `DISHWASHER` is recovered when its row separator is not full-page-width. The live historical Spec List and latest `spec-list.xlsx` contain `DISHWASHER` / `DBI364ID.S.AU` and `TAPWARE (KITCHEN)` / `TA9601CP`, with the false `TAPWARE (KITCHEN) BY CLIENT` section absent. Documented in `docs/BUG_IMPERIAL_V6_DISHWASHER_TAPWARE_DATA_LOSS_TASK.md`; tested in `tests/test_v6_titled_template_recovery.py`. The fix does not change the current Imperial primary `Tap` room-card/material-summary exclusion policy.
- Imperial v6 row assembly must keep wrapped single labels separate from true missing-row-separator blocks. Job 38 page 2 source text `BASE CABINETRY COLOUR / INCLUDES SINGLE CABINET ON BAR / BACK AREA` is one base-cabinetry colour row with row-local note text, not three rows. The extractor now preserves AREA overflow words in the AREA/SPECS gap, repairs that wrapped row before review-hint splitting, and leaves the finalized material value as `Polytec - Classic White Matt` with the qualifier retained only in `notes`. Fixed 2026-05-18 and production-verified on job 38 run 2371 (`Imperial v6`, build `local-89aa455a`): the latest `spec-list.xlsx` has one `BASE CABINETRY COLOUR` source row and no fake `INCLUDES SINGLE CABINET ON` or `BACK AREA` label rows. Documented in `docs/BUG_IMPERIAL_V6_BASE_CABINETRY_WRAPPED_LABEL_TASK.md`; tested in `tests/test_pdf_extractor_split.py` and `tests/test_imperial_v6_adapter.py`.
...
- Source-row data loss on non-room pages: job 38 run 2369 omitted source-backed page 5 `DISHWASHER` and page 8 `TAPWARE (KITCHEN)` rows from the v6 output. Fixed and production-verified on job 38 run 2370 with `App/services/pdf_to_structured_json.py` `extract_page_title()` row-label guard plus a scoped APPLIANCES-only template recovery pass behind an appliance AREA whitelist (`_APPLIANCE_AREA_WHITELIST`). Out of scope by design and remaining open: handle display dedupe / grouped handle summary supplier splitting, label/note clipping (`SINKWARE (KITCHE`, `Tap Landi`).
- Base cabinetry wrapped-label split: job 38 page 2 was incorrectly split into `BASE CABINETRY COLOUR`, `INCLUDES SINGLE CABINET ON`, and `BACK AREA` rows. Fixed 2026-05-18 with extractor-side wrapped-label repair and finalizer note-prefix dedupe, and production-verified on job 38 run 2371. Remaining open job 38 issues: handle display dedupe / grouped handle summary supplier splitting, label/note clipping (`SINKWARE (KITCHE`, `Tap Landi`).
