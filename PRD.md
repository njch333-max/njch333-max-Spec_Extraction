...
- Imperial raw Spec List room cards should prefer grouped handle rendering when `display_groups` is present: render supplier once as a group header and render the group's lines beneath it. When `display_groups` is absent, fall back to the existing flat `display_lines` rendering. This UI grouping does not change the underlying Material Summary aggregation path, which still consumes flat `display_lines`.
...
- Repeated `HANDLES` rendering on Kitchen room cards: a single `HANDLES` block can render multiple times for the same kitchen when the source table expresses several separate handle rows. Room cards remain readable but visually redundant.
