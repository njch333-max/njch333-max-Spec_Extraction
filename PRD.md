...
- Evoca structured adapter wiring is specified in `docs/EVOCA_ADAPTER_WIRING_SPEC.md`. The offline pure adapter module `App/services/evoca_structured_adapter.py` maps only `evoca_structured_v0` JSON into `SnapshotPayload`, preserves row-local provenance, keeps sections 16/18/19/21/22 out of snapshot output, and routes overloaded `Type` labels by parent group; it is still not wired into production parsing.
