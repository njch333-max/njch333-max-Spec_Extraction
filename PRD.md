...
- Imperial supplier-cell ownership must preserve clean supplier cells such as `By Imperial` in room-card raw rows and PDF-QA checklist values. Handle summary may drop suppliers for semantic grouping, but raw rows must still display the supplier cell when it exists in the PDF.
