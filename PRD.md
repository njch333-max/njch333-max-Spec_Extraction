...
- Evoca structured adapter wiring is specified in `docs/EVOCA_ADAPTER_WIRING_SPEC.md`. The offline pure adapter module `App/services/evoca_structured_adapter.py` maps only `evoca_structured_v0` JSON into `SnapshotPayload`, preserves row-local provenance, hard-excludes sections 16/18/19/21/22 from snapshot output, routes terminal values verbatim without retaining rooms by them, and routes overloaded `Type` labels by parent group; it is still not wired into production parsing.
