...
- Populate `Shelf` only when the same room's source text explicitly ties a material or finish to shelf shelving, such as `Open Shelving ... White Melamine` or `Shelves ... Polytec ...`. Do not infer it from rail-only rows, generic fit-out notes, unrelated material fields, or shelf text that belongs to another room on the page.
