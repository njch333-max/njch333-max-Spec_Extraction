...
- Imperial raw Spec List room-card `material_rows` must also render row-local `notes` without changing summary behavior: when a row's visible payload is a single line, append notes inline as `text - (notes)`; when the payload is multi-line, render notes as a trailing `(notes)` line after the existing row content. Empty notes render nothing, and this rule does not modify `Material Summary`.
