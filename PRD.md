...
- Evoca real-PDF e2e coverage is defined in `tests/test_evoca_end_to_end.py`; it runs only when private PDFs are supplied through `EVOCA_E2E_PDF_DIR` or the ignored `tests/fixtures/evoca_real_pdfs` directory, so customer PDFs are not committed to source control. `tools/run_evoca_real_pdf_e2e.ps1` is the gating runner and fails when any of the nine validated PDFs are missing unless `-AllowSkip` is explicitly used. GitHub CI support uses a self-hosted Windows runner with an external private fixture folder; setup is documented in `docs/EVOCA_PRIVATE_E2E_CI_SETUP.md`.
