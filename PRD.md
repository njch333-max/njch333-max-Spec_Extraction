...
- Evoca real-PDF e2e coverage is defined in `tests/test_evoca_end_to_end.py`; it runs only when private PDFs are supplied through `EVOCA_E2E_PDF_DIR` or the ignored `tests/fixtures/evoca_real_pdfs` directory, so customer PDFs are not committed to source control.
