...
- Evoca structured adapter wiring is specified in `docs/EVOCA_ADAPTER_WIRING_SPEC.md`. The future adapter must map only `evoca_structured_v0` JSON into `SnapshotPayload`, preserve row-local provenance, keep sections 16/18/19/21/22 out of snapshot output, and route overloaded `Type` labels by parent group before any production wiring is approved.
