...
9a. Builder routing, source-PDF review scope, and regression ownership must come from the website job's assigned Builder record, not from PDF header text such as `Client`, `Builder`, logos, or sheet styling. If the app classifies a job as `Imperial`, treat it as Imperial even when the uploaded PDF visually resembles another builder's delegated selection sheet.
...
14a. Builder × page-family extraction matrix is now explicit:
    ...
    - `Yellowwood`
      ...
      - dedicated Yellowwood structured-parser work should follow the Evoca staged process model: standalone extractor and source-native schema first, private real-PDF e2e tests next, adapter spec and pure adapter after source-PDF review, and production wiring only after explicit approval. Do not copy Evoca section numbering or hard-exclusion rules into Yellowwood without Yellowwood source-PDF evidence.
    - `Simonds`
    ...
    - `Evoca`
      ...
      - Standalone Evoca output intentionally skips sections 16/18/19/21/22, while still recognizing those headings as hard boundaries so excluded rows cannot leak into included sections; observed Evoca PDFs may label the excluded plumbing/gas and air-conditioning headings as either `18 PLUMBING & GAS` / `19 AIR-CONDITIONING` or the older `18 AIR-CONDITIONING` / `19 PLUMBING & GAS`, and both forms are hard boundaries.
      - Evoca adapter wiring must follow `docs/EVOCA_ADAPTER_WIRING_SPEC.md`: start from `evoca_structured_v0` JSON, hard-exclude sections 16/18/19/21/22 from snapshot output, strip loaded page text from `source_documents`, route terminal values verbatim without retaining rooms by them, keep Benchtops `Colour & Finish` / `Finish` in canonical bench-top fields, derive `Drawers` / `Hinges` soft-close states from source-backed Section 15 cabinet notes, route overloaded `Type` labels by parent group, preserve row-local provenance, and do not hide wrong parser values in downstream mapping. Spec List may format Evoca structured room values for legacy-style display, hide raw `special_sections` cards, suppress incomplete Section 20 wet-area accessory/shower/bath/toilet display rows, keep source-backed handle decisions such as `Client to supply & install after handover` or `Finger Pulls to Kitchen` while filtering exact `#N/A`/`Not Applicable` markers, and derive visible flooring/splashback rows from included finish sections, but this must remain presentation-only and must not mutate stored source rows or provenance. The pure adapter lives in `App/services/evoca_structured_adapter.py`; production dispatch is the `build_spec_snapshot` Evoca structured fast path, gated by the website Builder record plus `SPEC_EXTRACTION_ENABLE_EVOCA_STRUCTURED`, which defaults off for controlled rollout, with legacy fallback on flag-off, unsupported source counts, extractor errors, or no source-backed evidence. Evoca Job Workspace runs also expose explicit per-run `Evoca Structured` and `Heuristic Only` buttons; those per-run modes override the global flag only for that run and must not mutate environment settings.
      - Evoca structured Raw Snapshot Excel export uses a source-native workbook shape instead of the generic `By Section` review workbook: `Summary` plus one sheet per included source section, with `Page / Order / Room / Group / Label / Value / Anchor / Source Text` columns, room banners, anchor styling, and no raw `Special Sections` sheet. This is an export presentation surface only and must be built from stored snapshot provenance without re-running the PDF parser.
      - Evoca real-PDF e2e tests live in `tests/test_evoca_end_to_end.py`. The test harness may use private PDFs from `EVOCA_E2E_PDF_DIR` or `tests/fixtures/evoca_real_pdfs`, but customer PDFs must stay ignored and must not be committed. Use `tools/run_evoca_real_pdf_e2e.ps1` for a gating run; it requires all required private Evoca regression PDFs unless `-AllowSkip` is explicitly passed. GitHub CI support is documented in `docs/EVOCA_PRIVATE_E2E_CI_SETUP.md` and uses a self-hosted Windows runner with an external private fixture folder; `tools/setup_github_evoca_runner.ps1` validates that folder and configures the labeled runner after GitHub provides a short-lived registration token.
    - `Clarendon`
...
15a. Clarendon deterministic polish may restore a room dropped by earlier retention only when the selected `Drawings and Colours` room-master schedule overlay contains true material fields for that same room; AFC/supplement fixture or flooring overlays still must not create rooms.
...
22a. The next Yellowwood dedicated parser cycle should start from `docs/YELLOWWOOD_NEXT_CHAT_HANDOFF_2026-05-03.md` and a new parser spec. Treat Evoca as the process blueprint, not as the business-rule source: Yellowwood acceptance remains concrete source titles, row-local material ownership, retained-room evidence, room-local overlays, and wet-area noise suppression.
...
29. For Imperial structure work, treat `grid boundary recovery` as the first truth layer. If `AREA / ITEM` is contaminated by `SPECS / DESCRIPTION`, or merged-cell continuation crosses row boundaries, fix separator / row-assembly logic before relying on summary cleanup or source-PDF review.
...
28c-1. For Imperial v6 `HANDLES`, keep source-backed row-payload `display_lines` as the flat summary/fallback surface, but allow a separate HANDLES-only `display_groups` UI surface when supplier grouping is safe enough to express. The accepted grouped case includes equal-share multi-supplier rows where `M` handle spec lines divide evenly across `N` supplier lines in source order. Helper-produced rendered lines or flattened `display_value` are room-card display surfaces only and must not become `Material Summary` input. Do not add `display_groups` to non-HANDLES rows.
...
28e-1. Imperial handle summary is subitem-first. If a handle row has `handle_subitems`, summary and raw export grouping must use only subitem `summary_text` / `text`; subitem `raw_text`, `layout_value_text`, `page_text_handle_block`, or other provenance strings are evidence for repair/debug only and must not become summary material.
28e-1a. If an Imperial handle row has no usable `handle_subitems`, summary fallback must stay source-backed and deterministic in this order: source-backed v6 `display_lines`, synth/review `provenance.layout_value_text`, raw `supplier + specs_or_description + notes`, then `page_text_handle_block` as the last fallback. Helper-produced rendered lines and flattened `display_value` must not drive handle summary wording.
...
28e-3. For Imperial v6 raw Spec List presentation, `Material Summary -> Handles` may prefer HANDLES-only `display_groups` when they are present on a tagged v6 row. In that grouped path, one supplier/lines block is one distinct item, grouped entries dedupe only on exact `(supplier, tuple(lines))`, and grouped rendering must not spill into `Door Colours` or `Bench Tops`. When `display_groups` is absent, fall back to the existing flat handle-summary path.
28e-4. For Imperial v6 raw Spec List presentation, suppress same-source HANDLES subset rows before both room-card rendering and `Material Summary -> Handles` aggregation. If one same-room HANDLES row's normalized lines are a strict subset of another longer row, keep the canonical longer row and drop the subset derivative. This display-layer dedupe may treat `synthesized_from_room_handles` backfill rows as eligible subset rows, but it must not affect non-HANDLES rows or dedupe across non-v6 origins.
28e-4a. For Imperial v6 handle backfill inside `_imperial_reconcile_material_rows_with_room_fields(...)`, repeated builder-finalizer passes must be idempotent. If a room already contains a `synthesized_from_room_handles` row with the same normalized `provenance.layout_value_text`, skip re-emitting it. Keep this fix at the reconcile emit gate; do not widen it into finalizer-topology or handle-identity-helper changes unless separately approved.
28f. Imperial primary room cards and primary raw export/display keep only `Drawers`, `Hinges`, `Flooring`, and `Sink` beneath the raw material rows. `Tap` is intentionally excluded from Imperial primary display, primary summary, and primary raw export/display.
...
28j-1. Imperial raw Spec List room-card `material_rows` must render row-local `notes` in the template only: append ` - (notes)` inline only when the row's visible payload is one line and the notes string contains no `\n`; otherwise render a trailing notes line with `.row-note-multiline { white-space: pre-line; }` so source-backed line breaks remain visible. Empty notes render nothing, and this rule must not spill into `Material Summary`. For `synthesized_from_room_handles` rows specifically, helper-level display lines must leave notes out and keep them on the row object so the room-card notes layer renders that text once.
28j-2. For Imperial v6 room-card notes, when flattened `material_rows[].notes` is empty after parser finalization, `_flatten_imperial_material_rows` may fall back to the matching `rooms[].v6_review_rows[*].notes` only. Match must degrade conservatively `(area, page, row_order) -> (area, page) -> area`, refuse ambiguous matches, and must not rewrite suppliers, specs, display payloads, `Material Summary`, or stored snapshot rows.
...
28o-5. Imperial Phase 3B sinkware/appliance overlay is row-first and cluster-local. Taphole notes may cross room clusters only when the normalized fixture-base signature matches, or when multiple same-base generic sink rows share one sink-oriented note; never copy that note to a different fixture base, and clear sink-derived pseudo-`basin_info` on utility rooms. Appliance layout cells must drop image-column and `N / A - By others` placeholder contamination before model extraction while retaining `Specs - TBC` as a real source-backed placeholder row.
28o-6. Imperial sink/basin display cleanup must preserve source-equivalent supplier/mounting order. Do not render supplier inside the mounting phrase (`Sink Mounting - By Others Undermount`); render the row as mounting first, supplier second (`Sink Mounting - Undermount - By Others`). Deterministic PDF text-layer encoding glitches may be normalized only when the visual source is unambiguous, for example `MounƟng` to `Mounting`.
28o-7. Imperial sinkware/basin taphole and note carry-forward must be constrained by normalized fixture-base signature. A note such as `Note: Urbane Brass Taps` may be shared across matching basin rows, but not across different fixture bases; render `Taphole location` before the matching note when that is the source visual order.
28o-8. Imperial v6 appliance and sinkware/tapware continuation recovery must retain source-backed rows even when a real row appears before a repeated page heading or when a known `AREA / ITEM` label is glued to adjacent product/model text. Recover examples such as page-local `DISHWASHER` rows and glued `TA9601CPTAPWARE (KITCHEN)` only from source evidence; do not use this rule to invent absent fixture rows, change the Imperial `Tap` room-card exclusion policy, or broaden into cabinetry continuation / handle-summary cleanup. The 2026-05-18 implementation in `App/services/pdf_to_structured_json.py` enforces this in two places: (a) `extract_page_title()` skips lines matching `^(SINKWARE|TAPWARE)\s*\(...\)` so AREA-column row labels cannot start a new section, and (b) titled `APPLIANCES` pages run a scoped template-recovery pass restricted to AREA labels in `_APPLIANCE_AREA_WHITELIST`. Any extension to other titled pages or new appliance AREA labels must update the whitelist + tests in `tests/test_v6_titled_template_recovery.py` rather than relax the title-detection or recovery rules.
28o-9. Imperial v6 missing-row-separator repair must distinguish true multi-row blocks from wrapped single labels. A source row such as `BASE CABINETRY COLOUR / INCLUDES SINGLE CABINET ON BAR / BACK AREA` is one base-cabinetry colour row with a row-local note, not three rows. Keep AREA-to-SPECS gap overflow words such as `BAR`, move wrapped `INCLUDES...BACK AREA` qualifiers to `notes`, remove duplicate leading supplier text from `specs`, and prevent finalized door-colour descriptions from copying a prefix of the same note. Do not weaken the existing true multi-row split for rows such as `LED LIGHTING / PULL-OUT BIN / PULL-OUT SHELVES`.
...
29. User-visible PDF QA is retired. Do not reintroduce `Pending PDF QA` cards, `/pdf-qa` routes, QA signoff buttons, export locks, or snapshot-verification gates unless the user explicitly requests a new QA workflow.
30. Raw Snapshot Excel export is direct: `/jobs/{job_id}/spec-list.xlsx` should be available whenever the latest `raw_spec` snapshot exists. Historical run result pages remain read-only and must not expose Excel export.
31. Raw Snapshot Excel uses the Claude-style workbook shape: `Summary`, `By Section`, optional `Flagged`, and filtered `Material Summary` when Bench Tops / Door Colours / Handles rows exist. Preserve Claude's Arial font sizing, blue table headers, green section headers, yellow flagged rows, and section-header row layout; item rows should keep only the raw area/item label instead of a flattened `Section / Area` prefix. Imperial v6 exports prefer `rooms[].v6_review_rows` when present so original v6 item boundaries, row wording, and notes survive parser finalization, falling back to `rooms[].material_rows` for older snapshots; non-Imperial exports flatten room fields, appliances, special sections, others, and warnings into `By Section`.
31a. Formal job exports are no longer locked behind PDF QA. Generate and download actions must not depend on `snapshot_verifications.status`.
31b. Parser-accuracy work still requires source-PDF review of affected fields after a fresh live rerun. Older webpages and older snapshots are reference material only.
31c. Do not treat non-empty extracted values as sufficient parser-accuracy evidence; source-PDF comparison remains the acceptance source of truth.
...
4. PR descriptions must call out:
   ...
   - whether reruns and source-PDF review are required
5. Default Codex review focus is bug risk, parser regression, builder cross-contamination, user-workflow regressions, export regressions, and field-name drift across UI/export/storage.
...
7. For this project, default to `fix this bug` when a live issue is already specific and PDF-grounded. Prefer `review this PR` only when the change touches shared parser flow, grouped-row cleanup, builder finalizers, user workflow, or export state handling.
...
4. Verify `/api/health`.
5. If parsing logic changed, re-run the affected online job, confirm the latest run uses the new build, review affected fields against the source PDF, and only then close the task.
