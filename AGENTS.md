...
28j-1. Imperial raw Spec List room-card `material_rows` must render row-local `notes` in the template only: append ` - (notes)` inline only when the row's visible payload is one line and the notes string contains no `\n`; otherwise render a trailing notes line with `.row-note-multiline { white-space: pre-line; }` so source-backed line breaks remain visible. Empty notes render nothing, and this rule must not spill into `Material Summary`.
