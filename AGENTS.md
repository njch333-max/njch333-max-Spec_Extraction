...
28e-4b. For Imperial v6 raw Spec List presentation, if final `material_rows` collapsed multiple source HANDLE rows but same-room `v6_review_rows` still preserves the missing source rows, `_flatten_imperial_material_rows(...)` may supplement HANDLES-only missing rows from `v6_review_rows` before room-card rendering and `Material Summary -> Handles`. Match by source identity, visual identity, and content identity to avoid duplicates. Supplemented rows must keep the normal HANDLE room-card formatting: `SUPPLIER` as the bold continuous header, `SPECS / DESCRIPTION` as wrapped source lines, and `NOTES` as the row-level muted parenthetical note. Supplier-cell status lines such as `SUPPLIED BY CLIENT` and `INSTALLED BY IMPERIAL` stay in one continuous supplier header; they must not be split into separate supplier groups that own different specs lines. Do not apply this to non-HANDLES rows and do not mutate stored snapshot rows.
