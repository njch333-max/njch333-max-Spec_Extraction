...
14a. Builder × page-family extraction matrix is now explicit:
    ...
    - `Evoca`
      ...
      - Evoca adapter wiring must follow `docs/EVOCA_ADAPTER_WIRING_SPEC.md`: start from `evoca_structured_v0` JSON, hard-exclude sections 16/18/19/21/22 from snapshot output, route terminal values verbatim without retaining rooms by them, route overloaded `Type` labels by parent group, preserve row-local provenance, and do not hide wrong parser values in downstream mapping. The offline pure adapter lives in `App/services/evoca_structured_adapter.py` but remains disconnected from production dispatch until explicitly approved.
    - `Clarendon`
