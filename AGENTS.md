...
31c. Imperial PDF QA checklist rows must follow canonical `material_rows.row_order` before stale provenance order hints. If checklist order disagrees with the source table while room-card order is correct, fix checklist ordering before signing off.
