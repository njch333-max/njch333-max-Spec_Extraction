...
14a. Builder × page-family extraction matrix is now explicit:
    ...
    - `Evoca`
      ...
      - Evoca adapter wiring must follow `docs/EVOCA_ADAPTER_WIRING_SPEC.md`: start from `evoca_structured_v0` JSON, keep skipped sections skipped, route overloaded `Type` labels by parent group, preserve row-local provenance, and do not hide wrong parser values in downstream mapping.
    - `Clarendon`
