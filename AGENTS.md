...
28c-1. For Imperial v6 `HANDLES`, keep `display_lines` as the flat summary/fallback surface, but allow a separate HANDLES-only `display_groups` UI surface when supplier grouping is safe enough to express. The accepted grouped case includes equal-share multi-supplier rows where `M` handle spec lines divide evenly across `N` supplier lines in source order. Do not add `display_groups` to non-HANDLES rows.
