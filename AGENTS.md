...
14a. Builder × page-family extraction matrix is now explicit:
    ...
    - `Evoca`
      ...
      - Evoca real-PDF e2e tests live in `tests/test_evoca_end_to_end.py`. The test harness may use private PDFs from `EVOCA_E2E_PDF_DIR` or `tests/fixtures/evoca_real_pdfs`, but customer PDFs must stay ignored and must not be committed. Use `tools/run_evoca_real_pdf_e2e.ps1` for a gating run; it requires all nine validated PDFs unless `-AllowSkip` is explicitly passed. GitHub CI support is documented in `docs/EVOCA_PRIVATE_E2E_CI_SETUP.md` and uses a self-hosted Windows runner with an external private fixture folder; `tools/setup_github_evoca_runner.ps1` validates that folder and configures the labeled runner after GitHub provides a short-lived registration token.
    - `Clarendon`
