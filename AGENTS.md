...
14a. Builder × page-family extraction matrix is now explicit:
    ...
    - `Evoca`
      ...
      - Evoca real-PDF e2e tests live in `tests/test_evoca_end_to_end.py`. The test harness may use private PDFs from `EVOCA_E2E_PDF_DIR` or `tests/fixtures/evoca_real_pdfs`, but customer PDFs must stay ignored and must not be committed.
    - `Clarendon`
