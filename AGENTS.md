...
28j-1. Imperial raw Spec List room-card `material_rows` must render row-local `notes` in the template only: append ` - (notes)` inline when the row's visible payload is one line, and render a trailing notes line when the payload is multi-line. Empty notes render nothing, and this rule must not spill into `Material Summary`.
