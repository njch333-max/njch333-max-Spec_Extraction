...
24. `Shelf` is a first-class conditional material field. Populate it only when the source explicitly assigns a material or finish to shelf shelving; never infer it from rail-only rows or generic fit-out notes, and do not render it when blank.
